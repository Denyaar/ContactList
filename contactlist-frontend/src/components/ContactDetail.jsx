...
import { getContactById, updatePhoto } from "../api/ContactService";
...
const ContactDetail = ({ updateContact, updateImage }) => {
  ...
  const updateUserImage = async (file) => {
    try {
      ...
      setContact((prev) => ({ ...prev, imageUrl: `${prev.imageUrl}?updated_at=${new Date().getTime()}` }));
    ...
    <>
      ...
      <div className="profile">
        <div className="profile__details">
          <img src={contact.imageUrl} alt={`Profile photo of ${contact.name}`} />
      ...
      <form style={{ display: "none" }}>
        <input type="file" ref={inputRef} onChange={(event) => updateUserImage(event.target.files[0])} name="file" accept="image/*" />
      </form>
    </>
  );
};
